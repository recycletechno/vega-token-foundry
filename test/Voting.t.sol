...
/**
 * @title VotingPartialsTest
 * @dev Refactored tests
 * Run with: forge test --match-contract VotingPartialsTest
 */
contract VotingPartialsTest is Test {
    ...
    function setUp() public {
        ...
        voting = new Voting(address(token), address(nft));
    ...
    function testCannotVoteTwice() public {
        ...
        // console.log("Satoshi voted 1");
        
        // Second vote should fail
        vm.expectRevert("Already voted");
        voting.castVote(1, true);
        
        // console.log("Satoshi voted 2");
    ...
    /**
     * @dev Test voting power calculation
     */
    function testVotingPower() public {
        // Setup: Stake tokens with different years
        vm.startPrank(satoshi);
        token.approve(address(voting), 200 * tokenMultiplier);
        voting.stakeTokens(200 * tokenMultiplier, 4); // 4 years
        vm.stopPrank();
        
        vm.startPrank(vitalik);
        token.approve(address(voting), 100 * tokenMultiplier);
        voting.stakeTokens(100 * tokenMultiplier, 2); // 2 years
        vm.stopPrank();
        
        vm.startPrank(donald);
        token.approve(address(voting), 100 * tokenMultiplier);
        voting.stakeTokens(100 * tokenMultiplier, 1); // 1 year
        vm.stopPrank();
        
        // Create a vote
        vm.prank(owner);
        voting.createVote("Test voting power", block.timestamp + 1 days, 4000 * tokenMultiplier);
        ...
        // Check vote counts
        (, , , uint256 yesVotes, , ) = voting.votes(1);
        
        // Expected voting power:
        // Satoshi: 200 * (4^2) = 200 * 16 = 3200
        // Vitalik: 100 * (2^2) = 100 * 4 = 400
        // Donald: 100 * (1^2) = 100 * 1 = 100
        // Total: 3700
        uint256 expectedVotingPower = (200 * tokenMultiplier * 16) + (100 * tokenMultiplier * 4) + (100 * tokenMultiplier * 1);
        assertEq(yesVotes, expectedVotingPower, "Total voting power should match expected calculation");
    }
